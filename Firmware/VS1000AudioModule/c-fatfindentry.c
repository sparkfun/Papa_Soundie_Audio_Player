...
again:
  ...
  while (1)
  {
    ...
    if (fn == 0xe5 || fn == 0)
    { /* 0xe5 / 0x00 deleted or new entry */
    ...
    else
    {
      if ((FatGetByte (i + 11 /* Attr */ ) & 0xde) == 0)
      {
        ...
        /* Only check 8.0, if these match, it is not valid. This only restricts 
         * the selection of filename, it does not create duplicates. We can
         * then later have any suffix we want when we actually create the
         * entry, i.e. .OGG, .MP3 or .WAV . */
...
#endif
    getfirst:
      map->Read (map, minifatInfo.currentSector =
                 currentSector, 1, minifatBuffer);
    }
  }
}

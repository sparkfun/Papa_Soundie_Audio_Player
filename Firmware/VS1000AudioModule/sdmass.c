...
#endif
  /* We know this is called one block at a time by the USB ROM firmware, so
   * only prepare for that and save 20 words. */
...
{
  /* We know this is called one block at a time by the USB ROM firmware, so
   * only prepare for that and save 14 words. */
...

  {
...
#ifdef USE_HC
      c =
        MmcCommand (MMC_WRITE_MULTIPLE_BLOCK | 0x40, firstBlock << mmc.hcShift);
#else
    ...
    else
    {
      ...
      /* MUST not use SpiSendClocks(): some MMC/SD's don't support interrupted
       * XCS. */
...
//  BusyWait10();
  ...
  while (mmc.state != mmcNA && mmc.errors == 0)
  {
    ...
    {
      /* Do we seem to be detached? Note: Now also requires USBWantsSuspend()
       * to return TRUE! */
      if (USBIsDetached ())
      {
        break;
...
auto u_int16 NewDiskProtocolCommand (register __i2 u_int16 * cmd)
{
  ...
  {
    /* The next command after STOP UNIT will not be serviced fully because we
     * will turn ourselves off. */
    startUnit = 2;
  }
#endif
  ...
  {
    /* Poll MMC present by giving it a command. But only when idle, i.e. mmcOk. 
     */
#ifdef USE_MULTIPLE_BLOCK_WRITE
    /* with "if" it is a little faster - but more code */
    // if (mmc.nextWriteBlock != 0xffffffff)
...
#else
  /* fixes SCSI_READ_10 residue problem -- mainly for Linux compatibility, does 
   * not fix LBAB, but is shorter.. */

...
tryagain:
  ...
  i = MmcCommand (MMC_SEND_IF_COND /* CMD8 */  | 0x40,
                  0x00000122 /* 2.7-3.6V */ );  /* note: 0x22 for the right
                                                 * CRC! */
#if DEBUG_LEVEL > 2
  puthex (i);
  puts ("=IF_COND");
                               ...
                               register __reg_a u_int32 sector)
{
  ...
  if (i != 0xfe)
  {
    ...
    {
      /* data retrieval failed: D4-D7 = 0 D3 = out of range D2 = Card ECC
       * Failed D1 = CC Error D0 = Error */
    }
    SpiSendClocks ();
    return 1;
  }
  ...
  /* We force a call of user interface after each block even if we have no idle
   * CPU. This prevents problems with key response in fast play mode. */
  IdleHook ();
...
#if 0
struct FsMapper *FsMapMmcCreate (struct FsPhysical *physical, u_int16 cacheSize)
{
  PERIP (GPIO0_MODE) &= ~(MMC_MISO | MMC_CLK | MMC_MOSI | MMC_XCS);
  PERIP (GPIO0_DDR) = (PERIP (GPIO0_DDR) & ~(MMC_MISO))
    | (MMC_CLK | MMC_MOSI | MMC_XCS) | GPIO0_SD_POWER;
  PERIP (GPIO0_ODATA) = (PERIP (GPIO0_ODATA) & ~(MMC_CLK | MMC_MOSI)) | (MMC_XCS | GPIO0_CS1);  /* NFCE
                                                                                                 * high
                                                                                                 */
#if DEBUG_LEVEL > 1
...
void IterateFilesCallback (register __b0 u_int16 * name)
{
  register int i;
#ifdef TEST_WRITE
  /* If dirName is not NULL, capture some data required to update the file
   * entry. Otherwise perform the normal operation. */
  if (dirName)
  {
    if (!memcmp (dirName, name, 4))
    { /* 8 characters match */
...
u_int16 FsMapMmcWrite (struct FsMapper *map, u_int32 firstBlock,
                       u_int16 blocks, u_int16 * data)
{
  /* We know this is called one block at a time by the USB ROM firmware, or our
   * own code, so only prepare for that and save 14 words. */
  {
    register u_int16 c;
    if (mmc.state == mmcNA || mmc.errors)
      return 0;
...
auto u_int16 FatOpenFileWrite (u_int16 * name, u_int32 suffix)
{
  ...
  /* Open to get the fragments and other data correct. We already know that it
   * should exist. */
                           ...
                           register __c0 s_int16 byteSize)
{
  ...
  while (byteSize)
  {
    ...
    /* Prevent writing off the end of the allocated space of a file. This
     * requires that the file size will be fixed in FatCloseFile. */
    if (minifatInfo.filePos + writeAmount > 512 * minifatFragments[0].size)
    {
      writeAmount = minifatFragments[0].size * 512 - minifatInfo.filePos;
      if (writeAmount <= 0)
      {
        return srcIdx - byteOff;
      /*EOF*/}
    }
...
auto s_int16 FatCloseFile (void)
{
  ...
  { /* Update file size if possible */
    ...
    /* bytes 28,29,30,31 in the directory entry are the file size in
...
//    keyOldTime = -32767; /* ignores the first release of KEY_POWER */
   ...
   * automatically deletes mapper, creates a read-write FLASH mapper, and
   * restores read-only FLASH mapper at exit.
   *
   * We need to replace the main loop because we want to detect MMC/SD
   * insertion and removal.
   *
   * We do not hook the MassStorage, because we can call our own version from
   * the main loop without using the RAM hook. This saves some instruction
   * words. */
#if 0
...
//    player.volume = -24; /* "max" volume */
  ...
  while (1)
  {
...
#ifdef USE_USB
    /* If USB is attached, call our version of the MassStorage() handling
     * function. */
    if (USBIsAttached ())
    {
      PERIP (SPI0_CONFIG) = SPI_CF_MASTER | SPI_CF_DLEN8 | SPI_CF_FSIDLE1;
    ...
    if (InitFileSystem () == 0)
    {
      ...
      {
        /*
         * To be able to update the file size, we use IterateFiles() /
         * IterateFilesCallback() and some internal structures to locate the
         * file entry to be able to update it later.
         *
         * This code with the changes in IterateFilesCallback() finds the
         * directory sector and entry this file resides in, so we can
         * optionally fix the file size later. */
      ...
      if (OpenFile (0) < 0 && !memcmpY (minifatInfo.fileName, sdupdate, 8 / 2))
      {
        /* Note: The maximum size for the program image is 8192 bytes. */
        if (ReadFile (mallocAreaX, 0, 0x2000))
        {
          Disable ();
          BootFromX (mallocAreaX + 8);  // Exit to new image
          /* BootFromX returns only in case of invalid prg file. Program
           * execution would be unsafe after failed BootFromX so restart using
           * watchdog reset. */
          USEX (WDOG_CONFIG) = 2;
      ...
      {
        /* If no files found, output some samples. This causes the idle hook to
         * be called, which in turn scans the keys and you can turn off the
         * unit if you like. */
#ifdef USE_DEBUG
        puts ("no .ogg");
#endif
      ...
      /*
       * Currently starts at the first file after a card has been inserted.
       *
       * Possible improvement would be to save play position to SPI EEPROM for
       * a couple of the last-seen MMC/SD's. Could also save the play mode,
       * volume, and earspeaker state to EEPROM. */
              ...
              *bf = 0;
              if (cmd == '\n' || bf == &buffer[79])
              {
                ...
                case 'L':
                  /* List files */
#if 0
                  /* Set suffix to NULL to list all files TODO: use a
                   * parameter? */
                ...
                case 'T':
                  { /* Type by name */
                    ...
                    if (num != 0xffffU)
                    {
                      ...
                      while (l = ReadFile (buffer, 0, 2 * sizeof (buffer)))
                      {
                        register int i;
                        for (i = 0; i < l / 2; i++)
                        {
      ...
      playloop:
        ...
        /* If random play is active, get the next random file, but try to avoid
    ...
    {
      /* If not a valid FAT (perhaps because MMC/SD is not inserted), just send
       * some samples to audio buffer and try again. */
#ifdef UART_CONTROL
      putstrp ("\pnofat\n");
#endif
    noFSnorFiles:
      ...
      /* When no samples fit, calls the user interface -- handles volume

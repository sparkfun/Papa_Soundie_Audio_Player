...
void MyInitUSBDescriptors(u_int16 initDescriptors){
  ...
  /* Win2000 is confused about composite device with both
     mass storage and audio device. It just stops talking to
     the mass storage after the first Request Sense.
     Win7 is only slightly better. It seems to lock up if audio
...
/*REMS: Command+2dummy+1addr / manufacturer ID / device ID */
     ...
     Macronix.*/
  {
      ...
      SPI_MASTER_8BIT_CSHI; 
#ifdef UART_CONTROL
      if ((status & 0xff00) == 0xc200) { /*Macronix*/
          putstrp("\pMacronix\n");
      } else {
          putstrp("\pUnknown manufacturer\n");
      }
...
auto u_int16 NewDiskProtocolCommand(register __i2 u_int16 *cmd) {
    ...
    /*Note: command length is in hi byte of cmd[0],
      so bytes of the SCSI commands are:
              hi  lo
...
void MyPowerOff(void) {
    /* Because there is a large capacitor on CVDD, we have to make
       otherwise certain that the voltage does not rise above the
       power-on-reset level when power consumption drops before
       the regulators have been properly shut down.

       In this case we bring the SPI FLASH chip select low, so that
       IOVDD is dropping quickly, and prevents restart by preventing
       the oscillator circuit from running.
    */
    PERIP(GPIO1_DDR)   |=  1;
    PERIP(GPIO1_MODE)  &= ~1;
...
//  SetHookFunction((u_int16)OpenFile, Fat12OpenFile);
  ...
  if (USBIsAttached()) {                 
      if ((PERIP(GPIO0_DDR) & (1<<14)) == 0 &&
          ...
          putstrp("\pSD\n");
#endif/*UART_CONTROL*/
          ResetIRAMVectors(); /*Doesn't set variables to defaults, only jump vectors*/
